...
def normalizar_texto(texto):
    """Quita acentos y convierte a mayúsculas para comparaciones consistentes."""
    if not texto: return ""
...
def generar_html(productos, protocolos):
    # Ordenar productos alfabéticamente ignorando acentos
    productos.sort(key=lambda x: normalizar_texto(x['nombre'].strip()))
    ...
    for p in productos:
        ...
        # Usar texto normalizado para extraer la letra de agrupación (Á -> A)
        nombre_norm = normalizar_texto(nombre)
        letra = nombre_norm[0] if nombre_norm else '#'
    ...
    <div class="book-container">
        ...
        <section class="cover" id="inicio" style="position: relative; overflow: hidden; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 95vh; padding: 6rem 2rem;">
            ...
            <div style="z-index: 10; text-align: center; width: 100%;">
                ...
                <h2 style="font-weight: 400; letter-spacing: 4px; font-size: 1.6rem; color: var(--text-main); margin-bottom: 2.5rem; line-height: 1.4; text-align: center;">Mundo homeopático</h2>
...
def generar_indice_lista_completa(productos):
    ...
    for p in productos:
        ...
        # Normalizar la letra para agrupar (Á -> A)
        nombre_norm = normalizar_texto(nombre)
        letra = nombre_norm[0] if nombre_norm else '#'
        if not letra.isalpha(): letra = '#'
        
        # Insertar cabecera de letra si cambia
        if letra != current_letter:
            current_letter = letra
            items.append(f'<div style="margin-top: 1.5rem; margin-bottom: 0.5rem; font-weight: 800; color: var(--primary); font-size: 1.4rem; border-bottom: 1px solid var(--border); page-break-inside: avoid;">{current_letter}</div>')
        
        pid = slugify(nombre)
        items.append(f'<div style="margin-bottom: 2px; padding-left: 0.5rem;"><a href="#prod-{pid}" style="color: var(--text-main); text-decoration: none;">• {nombre}</a></div>')
    return "\n".join(items)
